...
async function getSummary(urlSum) {
  axios.get(urlSum).then((urlResponse) => {
    ...
    $("tr.Bxz\\(bb\\)").each((i, element) => {
      const link = $(element).find("td.Ta\\(end\\)").text();
      stats.push(link);
      console.log(link);
    });
...
async function getFinancials(urlFin) {
  axios.get(urlFin).then((urlResponse) => {
    ...
    $("div.Ta\\(c\\).Py\\(6px\\).Bxz\\(bb\\).BdB").each((i, element) => {
      const link = $(element).find("span").text();
      profits.push(link);
      //  console.log(link)
    });
...
async function getPassed() {
  ...
  if (ass / lia > 2) {
    testsPassed++;
    console.log(
      "CRITERION [4] MET: ASSET:LIABILITY RATIO IS 2:1 OR BETTER (ratio is " +
        ratio +
        ")"
    );
  } else {
    console.log(
      "CRITERION [4] FAILED: ASSET:LIABILITY RATIO IS NOT 2:1 OR BETTER(ratio is " +
        ratio +
        ")"
    );
  }
